...
class Script:
    ...
    def eof(self):
        return self.line >= len(self.lines) or (
            self.line >= len(self.lines) - 1 and self.col >= len(self.lines[self.line])
        )
    ...
    def add_lines(self, lines):
        ...
        *Add* lines to script by appending to the current list.
        ...
        for line in lines:
            res = self.command_parser.parse(line)
            if res is not None and res["name"] in ["include"]:
                for filename in res["args"]:
                    self.add_lines(filename)
                continue
