...
def make_interactive_shell(shell):
    ...
    if shell[0] is None:
        raise click.RuntimeError(
            "Could not find working shell for '{oshell}'. Check that it is in your path."
        )
...
@click.option(
    "--no-statusline/--statusline",
    help="disable/enable status line (the statusline may interfere with some full screen applications, such as vim). status line is enabled by default.",
)
@click.option("--line-mode", "-l", is_flag=True, help="Start script in line mode.")
@click.option(
    "--startup-command",
    multiple=True,
    help="Send TEXT to the terminal before starting input (a \\r will be appended). May be given multiple times.",
)
@click.option(
    "--monitor/--no-monitor",
    help="Run a server to send status information to gsc monitor clients (such as gsc-monitor).",
)
@click.option(
    "--monitor-hostname",
    default="auto",
    help="Specify the hostname to use for the gsc monitor server (default auto).",
)
@click.option(
    "--no-render",
    is_flag=True,
    help="Do not perform variable expansion in the script file before running. Variables in the script are identified with %{VarName} (so that they do not interfere with shell variables ${VarName} that may be used in the script). By default, environment variables will be expanded.",
)
@click.option(
    "--multi-char-keys",
    default="auto",
    help="Specify a list of multi-character keypresses that should be detected in scripts and sent as single input to the terminal. Currently only supports 'auto', which will use the `blessed` module to add all key press escape sequences listed in terminfo.",
)
@click.option(
    "--keymap",
    multiple=True,
    default=[],
    help="Additional key mappings to use when loading a script. Keymaps are strings (i.e. <Tab>) in the script that get turned into key press sequences (i.e. \t). These can be used to add control commands to the script (i.e. move the cursor to the left two spots).",
)
@click.option(
    "--print-keymaps",
    is_flag=True,
    help="Print all keymaps",
)
def gsc(
    script,
    shell,
    ...
    session.script.add_keymap("<tab>", "KEY_TAB")
    session.script.add_keymap("<bksp>", "KEY_BACKSPACE")
    session.script.add_keymap("<esc>", "KEY_ESCAPE")
    session.script.add_keymap("<up>", "OA")
    session.script.add_keymap("<down>", "OB")
    session.script.add_keymap("<left>", "OD")
    session.script.add_keymap("<right>", "OC")
    session.script.add_keymap("<enter>", "KEY_ENTER")
    session.script.add_keymap("<entr>", "KEY_ENTER")
    session.script.add_keymap("<del>", "KEY_DELETE")

    # add mappings from the user.
    for mapping in keymap:
        k, v = map(lambda x: x.strip(), mapping.split("="))
        session.script.add_keymap(k, v)
    session.script.expand_keymaps()

    if print_keymaps:
        print(
            "These text string will be replaced with the following control sequences with the script before it is ran.",
            end="\n\r",
        )
        for k in session.script.keymap.mappings:
            v = session.script.keymap.mappings[k]
            print(f"'{k}' -> {v!r}", end="\n\r")  # we are in raw output mode...
        session.cleanup()
        sys.exit(0)

    session.set_statusline(not no_statusline)
    if monitor:
        session.monitor_server_hostname = (
            get_my_ip() if monitor_hostname == "auto" else monitor_hostname
        )
...
@click.option(
    ...
    "-k",
    is_flag=True,
    help="Use keypress event driver instead of characters from stdin.",
)
...
@click.command(
    help="A gsc monitor client to status information about the current gsc session."
)
@click.version_option()
@click.option(
    "--remote-hostname",
    "-r",
    default="localhost",
    help="The remote server (running gsc) hostname.",
)
@click.option(
    "--local-hostname",
    "-l",
    default="auto",
    help="The local server (running gsc-monitor) hostname.",
)
@click.option(
    "--port",
    "-p",
    help="Specify the (local) port to use for communicating with gsc session.",
)
@click.option(
    "--before-context",
    "-B",
    default=4,
    help="Specify the number of lines of context to display before the current line.",
)
@click.option(
    "--after-context",
    "-A",
    default=10,
    help="Specify the number of lines of context to display after the current line.",
)
@click.option("--debug", "-d", is_flag=True, help="Log debug messages.")
@click.option("--verbose", "-v", is_flag=True, help="Log info messages.")
def gsc_monitor(
    remote_hostname, local_hostname, port, before_context, after_context, debug, verbose
):
    logger = None
    if verbose or debug:
        logger = logging.getLogger()
    ...
    try:
        ...
        print(
            "ERROR: could not import 'urwid' module. Please install it and try again."
        )
        sys.exit(1)
    ...
    def message_handler():
        ...
        upcomming_lines = state["lines"][
            l + 1 : min(len(state["lines"]), l + 1 + after_context)
        ]
...
@click.option(
    ...
    "-p",
    default=3000,
    help="Specify the (local) port to lisiten for new clients on.",
)
    ...
    while not exit:
        ...
        if ready:
            ...
            mesg = {
                "desc": "test message",
                "mode": "Modes.Insert",
                "lines": ["pwd", "ls", "cat tmp.txt", "gnuplot", "exit"],
                "pos": (2, 5),
            }
